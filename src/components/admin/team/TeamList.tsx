...
import { inviteStaff, removeStaff, changeUserRole, toggleStaffStatus, updateStaffPhone, toggleStaffService, toggleStaffSkill } from '@/app/admin/team/actions'
import { toast } from 'sonner'
import { User, Mail, Trash2, Plus, ShieldCheck, Clock, ChevronDown, Tablet, Scissors, Crown, Eye, EyeOff, Phone, Check, Loader2, Sparkles, Briefcase, Tag, LayoutGrid } from 'lucide-react'
...
export default function TeamList({
    ...
    const staffLabelPlural = vocabulary.staff_plural
    ...
    const handleSkillToggle = async (memberId: string, serviceId: string, currentEnabled: boolean) => {
        ...
        // Usar la nueva tabla staff_skills para el wizard avanzado
    ...
    const handleToggle = async (memberId: string, field: 'is_active_barber' | 'is_calendar_visible' | 'staff_category' | 'role_alias', currentValue: any) => {
        const key = `${memberId}-${field}`
        setTogglingField(key)
        // Note: currentValue is the NEW targeted value in actions, but here we pass the current one to toggle it
                            ...
                            <form action={handleInvite} className="flex flex-col gap-3">
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    <div className="relative w-full sm:w-48">
                                        ...
                                        <select
                                            name="staff_category"
                                            className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black outline-none transition-all appearance-none"
                                        >
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        name="role_alias"
                                        type="text"
                                        placeholder="Alias personalizado (opcional: ej. Terapeuta Senior)"
                                        className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black outline-none transition-all text-sm"
                                    />
                                </div>
                            </form>
                                        ...
                                        <div className="flex items-center gap-4 min-w-0 flex-1">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {member.role_alias ? (
                                                        <span className="text-[10px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded font-black uppercase">
                                                            {member.role_alias}
                                                        </span>
                                                    ) : (
                                                        <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-black uppercase">
                                                            {CATEGORY_OPTIONS.find(c => c.value === member.staff_category)?.label || member.staff_category}
                                                        </span>
                                                    )}
                                                    {(currentUserRole === 'owner' || currentUserRole === 'super_admin') && (
                                                        <p className="text-xs text-gray-400 truncate opacity-60">{member.email}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-6">
                                            ...
                                            <div className="flex items-center gap-3 w-full lg:w-auto">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <select
                                                        value={member.staff_category}
                                                        onChange={(e) => handleToggle(member.id, 'staff_category', e.target.value)}
                                                        disabled={togglingField === `${member.id}-staff_category`}
                                                        className="text-[10px] font-bold uppercase tracking-wider bg-transparent border-none p-0 focus:ring-0 cursor-pointer text-gray-500 hover:text-black transition-colors"
                                                    >
                                                        {CATEGORY_OPTIONS.map(opt => (
                                                            <option key={opt.value} value={opt.value}>{opt.label}</option>
                                                        ))}
                                            ...
                                            <div className="flex items-center gap-3 w-full lg:w-auto flex-1">
                                                ...
                                                <div className="flex flex-col flex-1">
                                                    ...
                                                    <input
                                                        type="text"
                                                        defaultValue={member.role_alias || ''}
                                                        onBlur={(e) => handleToggle(member.id, 'role_alias', e.target.value)}
                                                        placeholder="Ej. Terapeuta Senior"
                                                        className="text-[10px] font-bold uppercase tracking-wider bg-transparent border-b border-transparent focus:border-indigo-200 outline-none p-0 text-gray-500 focus:text-black transition-all"
                                                    />
                                                </div>
                                            </div>
