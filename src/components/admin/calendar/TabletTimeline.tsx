...
import { cancelBookingAdmin, quickCheckout, stretchBooking } from '@/app/admin/bookings/actions';
...
export default function TabletTimeline({
    bookings,
    staff,
    services,
    tenantId,
    ...
    const [stretchState, setStretchState] = useState<{ id: string, extraHeight: number } | null>(null);
    ...
    // Stretch Handlers Let
    const handlePointerDownResize = (e: React.PointerEvent, booking: PosBookingData) => {
        e.stopPropagation();
        startYRef.current = e.clientY;
        setStretchState({ id: booking.id, extraHeight: 0 });
        e.currentTarget.setPointerCapture(e.pointerId);
    };

    const handlePointerMoveResize = (e: React.PointerEvent, booking: PosBookingData) => {
        if (stretchState?.id === booking.id) {
            e.stopPropagation();
            const deltaY = Math.max(0, e.clientY - startYRef.current);
            const snappedDelta = Math.floor(deltaY / 30) * 30; // 15 mins steps
            if (snappedDelta !== stretchState.extraHeight) {
                setStretchState({ id: booking.id, extraHeight: snappedDelta });
            }
        }
    };

    const handlePointerUpResize = async (e: React.PointerEvent, booking: PosBookingData) => {
        if (stretchState?.id === booking.id) {
            e.stopPropagation();
            e.currentTarget.releasePointerCapture(e.pointerId);
            const extraHeight = stretchState.extraHeight;
            setStretchState(null);

            const extraMins = (extraHeight / CELL_HEIGHT) * 60;
            if (extraMins > 0) {
                const oldEndDate = toZonedTime(booking.end_time, TIMEZONE);
                const newEndDate = new Date(oldEndDate.getTime() + extraMins * 60000);

                const newEndTimeStr = format(newEndDate, "yyyy-MM-dd'T'HH:mm");

                const toastId = toast.loading('Extendiendo cita...');
                const res = await stretchBooking(booking.id, newEndTimeStr);

                if (res?.conflict) {
                    toast.dismiss(toastId);
                    if (res.resolutionType === 'REASSIGN') {
                        toast.success(res.message, { duration: 5000 });
                    } else {
                        toast.error(res.message, { duration: 10000 });
                        setWaModal({ isOpen: true, variant: 'cancel', clientName: res.conflictingBooking?.guest_name || 'Cliente', clientPhone: null });
                    }
                } else if (res?.success) {
                    toast.success('Cita extendida', { id: toastId });
                    router.refresh();
                } else {
                                ...
                                <div className="relative z-0" style={{ height: `${(effectiveEnd - effectiveStart + 1) * CELL_HEIGHT}px` }}>
                                    {dailyBookings
                                        ...
                                        .map(booking => {
                                            ...
                                            const isStretching = stretchState?.id === booking.id;
                                            const baseHeight = parseFloat(height);
                                            const activeHeight = isStretching ? baseHeight + stretchState.extraHeight : baseHeight;
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isStretching && "z-50 ring-2 ring-blue-500 shadow-xl opacity-95 transition-none cursor-ns-resize"
                                                    )}
                                                    style={{ top, height: `${activeHeight}px`, minHeight: '36px' }}
                                                >
                                                    <div className="flex flex-col h-full justify-between overflow-hidden p-2 pb-4">
                                                        <div>
                                                            ...
                                                            <div className={cn(
                                                                "text-[10px] opacity-70 truncate mt-0.5 flex items-center justify-between",
                                                                soloStaffId && "text-xs"
                                                            )}>
                                                                <span className="truncate">{serviceName}</span>
                                                                {(booking as any).is_any_staff ? (
                                                                    <span className="text-[8px] bg-black/5 text-gray-600 px-1 py-0.5 rounded font-bold uppercase ml-1 shrink-0">Flexible</span>
                                                                ) : (
                                                                    <span className="text-[8px] bg-yellow-500/10 text-yellow-700 px-1 py-0.5 rounded font-bold uppercase ml-1 shrink-0">Preferencia</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    ...
                                                    {/* DRAG HANDLE FOR STRETCHING */}
                                                    {['pending', 'confirmed'].includes(booking.status) && (
                                                        <div
                                                            className="absolute bottom-0 left-0 right-0 h-4 cursor-ns-resize flex items-end pb-1 justify-center bg-transparent group hover:bg-black/5 transition-colors z-[100] touch-none"
                                                            onPointerDown={(e) => handlePointerDownResize(e, booking)}
                                                            onPointerMove={(e) => handlePointerMoveResize(e, booking)}
                                                            onPointerUp={(e) => handlePointerUpResize(e, booking)}
                                                            title="Arrastrar para estirar cita"
                                                        >
                                                            <div className="w-8 h-1 rounded-full bg-black/20 group-active:bg-blue-500 transition-colors" />
                                                        </div>
                                                    )}
                                                </motion.div>
                                            );
